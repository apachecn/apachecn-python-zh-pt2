...
![](image/Formula_B15413_04_001.png)
...
假设我们有一个陈述或条件`p`，这个条件是`True`。假设我们有另一个陈述或条件`q`，它也是`True`。在真值表中，我们用符号表示*而不是*。所以， `p`是`False`，而 `q`也是`False`。那是因为如果`p`是`True`，那么*不是*`p`是*不是*`True`，换句话说就是`False`。符号![](image/AND_symbol.png)用于*和*，因此`p`*和*`q`写为`p ![](image/AND_symbol1.png) q`。符号`![](image/OR_symbol.png)`用于*或*，因此`p`*或*`q`写为`p ![](image/OR_symbol.png) q`。在表格式中，我们的真值表如下所示：

![Figure 4.1 – Truth table ](image/Figure_4.1_B15413.jpg)
...
![Figure 4.2 – if-else statement decision flowchart ](image/Figure_4.2_B15413.jpg)
...
![Figure 4.3 – if-elif-else statements decision flowchart ](image/Figure_4.3_B15413.jpg)
...
![Figure 4.4 – Basic Python operators ](image/Figure_4.4_B15413.jpg)
...
![Figure 4.5 – Indentation error ](image/Figure_4.5_B15413.jpg)
