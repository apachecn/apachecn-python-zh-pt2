...
![Figure 13.1 – Machine Learning and its relationships with input and output ](image/Figure_13.01_B15413.jpg)
...
![Figure 13.2 – Types of machine learning ](image/Figure_13.02_B15413.jpg)
...
*   **乙状结肠**可表述为![](image/Formula_B15413_13_001.png)，其中实数的输入为*x*。它返回一个介于–1 和 1 之间的值。
*   **Tanh**由*Tanh（x）*给出。为双曲线正切函数，实数输入*x*；它返回一个介于–1 和 1 之间的值。
*   **整流线性单元**（**ReLU**为分段线性函数。如果输入为正或 0，则其输出与输入相同。

下图显示了与上述每个激活功能相关的图表：

![Figure 13.3 – Activation functions ](image/Figure_13.03_B15413.jpg)

图 13.3–激活功能

模式通过**输入层**呈现给网络，该层与一个或多个**隐藏层**通信，其中实际处理通过加权**连接系统**完成。然后，隐藏层链接到**输出层**，在该层中输出答案，如下图所示：

![Figure 13.4 – Artificial Neural Network (ANN) model ](image/Figure_13.04_B15413.jpg)

图 13.4–人工神经网络（ANN）模型

如您所见，求和运算符（![](image/Formula_B15413_13_002.png)）获取输入值并在网络中移动并创建输出。输入新节点时，它们必须求和并返回单个值。激活函数本质上是*挤压*输入，并将其转换为一个输出值，该输出值表示一个节点应该贡献多少（即，节点应该何时激发）。一个节点被激活时被认为是被*激活*。它获取输出值并进行转换，以便下一个节点可以将其作为输入。这被称为**启动**。
...
![Figure 13.5 – Sample of unsupervised learning ](image/Figure_13.05_B15413.jpg)
...
*   三头肌皮褶厚度（mm）
*   2 小时血清胰岛素（μU/ml）
*   体重指数![](image/Formula_B15413_13_003.png)
*   糖尿病家系功能
*   年龄（年）
...
![Figure 13.6 – Variable explorer sample ](image/Figure_13.06_B15413.jpg)
...
![Figure 13.7 – Display of information after running a description algorithm ](image/Figure_13.07_B15413.jpg)
...
![Figure 13.8 – Dataset correlation graphic ](image/Figure_13.08_B15413.jpg)
...
![Figure 13.9 – Training the dataset output for algorithm printing input and output values ](image/Figure_13.09_B15413.jpg)
...
![Figure 13.10 – Data cycle in ML ](image/Figure_13.10_B15413.jpg)
...
![Figure 13.11 – Output of model run using the Keras library ](image/Figure_13.11_B15413.jpg)

图 13.11–使用 Keras 库运行模型的输出

请注意，在运行算法时，精度可能会有所不同。对算法进行几次测试，以查看窗口中的更改。运行精度模型后，我们使用`model.summary()`打印模型摘要，如下所示：

![Figure 13.12 – Model summary of the algorithm ](image/Figure_13.12_B15413.jpg)
...
![Figure 13.13 – Synthetic dataset scatterplot ](image/Figure_13.13_B15413.jpg)
...
![Figure 13.14 – BIRCH models using 2 and 3 clusters, respectively ](image/Figure_13.14_B15413.jpg)
...
![Figure 13.15 – K-means algorithm output ](image/Figure_13.15_B15413.jpg)
