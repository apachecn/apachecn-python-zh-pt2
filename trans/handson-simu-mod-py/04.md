...
![Figure 4.1 – Process of a random generator ](image/B15737_04_01.jpg)
...
![Figure 4.2 – A fraction of the circle ](image/B15737_04_02.jpg)
...
**   Now, we can check where the point we just generated falls:
    ...
    ![](image/Formula_04_003.jpg)

    如果*x*0*=y*0*=0*和*r=1*，则上一个等式变为：

    ![](image/Formula_04_004.jpg)

    这使我们了解，一个点落在圆形扇区内的必要条件是验证以下等式：

    ![](image/Formula_04_005.jpg)
 ...
 *![Figure 4.3 – Plot of the Pi estimation ](image/B15737_04_03.jpg)
...
该定理表明如下：考虑到大量变量![](image/Formula_04_019.png)，定义平均值的积分近似于预期值的估计值。让我们试着举个例子，让你明白这一点。我们将一枚硬币抛 10 次、100 次、1000 次，然后检查有多少次我们得到了头像。我们可以将获得的结果放入一个表格中，如下所示：

![4.4 – Table showing the results for coin toss](image/Figure_4.4_Table_showing_the_results_for_coin_toss.jpg)
...
3.  At this point, we draw a histogram of the data in order to verify that it is a uniform distribution:
    ...
    ![Figure 4.5 – Plot of the data distribution ](image/B15737_04_04.jpg)
...
![Figure 4.6 – Plot of the extracted samples ](image/B15737_04_05.jpg)
...
![Figure 4.7 – Graphs of the two distributions ](image/B15737_04_06.jpg)
...
![Figure 4.8 – Plot of the function ](image/B15737_04_07.jpg)

图 4.8–功能图

通过分析前面的图表，我们可以确定以下时间间隔：

*   x![](image/Formula_04_020.png)[a，b]
*   y![](image/Formula_04_020.png)[0，U]
 ...
 **![](image/Formula_04_011.jpg)

在计算该概率后，可以使用前面的等式追踪积分值，如下所示：

![](image/Formula_04_012.jpg)
...
5.  现在，我们可以计算这个值。为此，我们将使用一个`for`循环来迭代流程`N`*次。首先，我们生成两个随机数，如下所示：
    ...
    for k in range(N):
        ...
        y = ymin + (ymax - ymin) * random.random()
    ```* 
**   Both `x` and `y` fall within the rectangle of area *A*; that is, x ![](image/Formula_04_0201.png) [*a, b*] and y ![](image/Formula_04_0202.png) [*0, maxy*]. Now, we need to determine whether the following is true:

    ![](image/Formula_04_015.jpg)
    ...
    *   After iterating for `N` times, we can estimate the integral:
    ...
    ![](image/Formula_04_016.jpg)* 

 *我们所做的百分比误差等于以下值：

![](image/Formula_04_017.jpg)
...
![Figure 4.9 – Plot of numerical integration results ](image/B15737_04_08.jpg)
