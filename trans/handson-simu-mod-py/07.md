...
*   约束极小化问题，若*F![](image/Formula_07_039.png)R*n；也就是说，如果柔性集合*F*只是集合*R*n 的一部分
...
优化问题中存在全局最小点的一个充分但非必要的条件是 Weierstrass 定理通过以下命题表示的条件：设*F![](image/Formula_07_0391.png)R*n 为非空紧集。设*f*为*f*上的连续函数。如果是，则在*f*中存在*f*的全局最低点。
...
![Figure 7.1 – Local minimum conditions for the f(x) function ](image/B15737_07_01.jpg)
...
**下降法**是迭代法，从初始点 x0![](image/Formula_07_038.png)Rn 开始，生成一系列点{xn}n![](image/Formula_07_038.png)n，由以下等式定义：

![](image/Formula_07_002.png)

这里，![](image/Formula_07_003.png)向量是一个搜索方向，![](image/Formula_07_004.png)标量是一个称为步长的正参数，表示我们在![](image/Formula_07_005.png)方向上移动的距离。

在下降法中，选择![](image/Formula_07_006.png)向量和![](image/Formula_07_007.png)参数，以确保目标*f*函数在每次迭代时减少，如下所示：

![](image/Formula_07_008.png)

使用![](image/Formula_07_009.png)向量，我们取下降方向，即线*x=x*n*+*![](image/Formula_07_010.png)![](image/Formula_07_011.png)与梯度向量![](image/Formula_07_020.png)f（xn）形成钝角。这样，只要![](image/Formula_07_012.png)足够小，就可以保证*f*的减少。

根据![](image/Formula_07_013.png)的选择，有不同的下降方法。最常见的情况如下：
...
![Figure 7.2 – The convex function ](image/B15737_07_02.jpg)
...
![](image/Formula_07_014.png)

这两个表达式中的第一个称为关于*x*的偏导数，而第二个偏导数称为关于*y*。梯度为以下向量：

![](image/Formula_07_015.jpg)
...
![](image/Formula_07_018.png)

在这里，我们有以下几点：

*   ![](image/Formula_07_019.png)是学习率
*   ![](image/Formula_07_020.png)是坡度

对于足够小的![](image/Formula_07_021.png)值，该算法在有限的迭代次数内收敛到函数*f*的最小值。
...
![Figure 7.3 – The scenarios for the learning rate ](image/B15737_07_03.jpg)
...
3.  Now, we can draw a graph and display it:
    ...
    ![Figure 7.4 – The minimum point of the function ](image/B15737_07_04.jpg)
...
![Figure 7.5 – The procedure of finding a tangent ](image/B15737_07_05.jpg)
...
![](image/Formula_07_028.jpg)

在上一个等式中，我们有以下等式：

*   ![](image/Formula_07_029.png)是函数*f*的一阶导数
*   ![](image/Formula_07_030.png)is the second derivative of the function *f*
...
3.  Now, we can draw a graph and display it:
    ...
    ![Figure 7.6 – The minimum point of the function ](image/B15737_07_06.jpg)
...
![](image/Formula_07_034.png)

在前面的等式中，![](image/Formula_07_035.png)是数据集中随机选择的一个观测值的梯度。
...
3.  Let's visualize the `matyas` function:
    ...
    ![Figure 7.7 – Meshgrid plot to represent the function ](image/B15737_07_07.jpg)
