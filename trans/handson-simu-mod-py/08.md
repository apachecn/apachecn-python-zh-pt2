...
基于这些性质，我们可以把这个过程看作是大量极小增量的总和。在选择两个瞬间*t*和*s*后，随机变量*Y（s）-Y（t）*遵循正态分布，mean 为![](image/Formula_08_001.png)（s-t），方差为![](image/Formula_08_002.png)*（s-t）*，我们可以用以下等式表示：

![](image/Formula_08_003.jpg)

正态性假设在线性变换中非常重要。事实上，标准布朗运动的名称来源于标准正态分布的分布类型，参数为![](image/Formula_08_004.png)=0 和![](image/Formula_08_005.png)=1。

因此，可以说，具有单位均值和方差的布朗运动*Y（t）*可以表示为标准布朗运动的线性变换，根据以下等式：

![](image/Formula_08_006.jpg)

在前面的等式中，我们可以观察到以下情况：

*   ![](image/Formula_08_007.png)是标准的布朗运动。

该方程的弱点在于*Y（t）*假设负值的概率为正值；事实上，由于*Z（t）*具有独立增量的特征，可以采用负号，因此*Y（t）*的负性风险不是零。

现在，考虑足够小的时间间隔的 Brownian 运动（Wiener 过程）。该过程的无穷小增量如下表所示：

![](image/Formula_08_008.jpg)

前面的公式可以改写如下：

![](image/Formula_08_009.jpg)
...
![](image/Formula_08_011.jpg)

上述公式可用于迭代过程。对于非常大的*n*值，我们可以编写以下内容：

![](image/Formula_08_012.png)
...
![Figure 8.1: Brownian motion graph ](image/B15737_08_1.jpg)
...
![Figure 8.2: Amazon data on Yahoo Finance ](image/B15737_08_2.jpg)
...
![Figure 8.3: Amazon share graph ](image/B15737_08_3.jpg)
...
![Figure 8.4: Logarithmic values of the returns ](image/B15737_08_4.jpg)
...
![](image/Formula_08_022.jpg)

前面的式中的第二项，呃，叫做**日收益率**，根据 BS 模型，由下式给出：

![](image/Formula_08_023.jpg)
...
![](image/Formula_08_025.jpg)

![](image/Formula_08_026.jpg)
...
![Figure 8.5: Amazon trend graph ](image/B15737_08_5.jpg)
...
![Figure 8.6: Probability distribution of the portfolio returns ](image/B15737_08_6.jpg)
...
3.  Now we can recover the data:
    ...
    ![Figure 8.7- Statistics of the portfolios ](image/B15737_08_7.jpg)
    ...
    ![Figure 8.8: Graphs of the statistics ](image/B15737_08_8.jpg)
...
4.  After taking a quick look at the trend of stocks, the time has come to evaluate the returns:
    ...
    ![Figure 8.9: The stock returns DataFrame ](image/B15737_08_9.jpg)
