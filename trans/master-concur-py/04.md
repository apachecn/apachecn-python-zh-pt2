...
显然，打开和关闭外部文件与并发性并不十分相似。但是，我们前面提到，`with`语句作为上下文管理器，不仅用于管理文件描述符，而且通常用于管理大多数资源。如果您在阅读[第 2 章](02.html)、*Amdahl定律*时发现管理`threading.Lock()`类中的锁对象类似于管理外部文件，那么这就是两者之间比较的地方。
