...
计算![](assets/ebe28fe6-73f6-4ce6-b728-95d6d199340f.png)的算法似乎效率低下。我们稍后会解决这个问题。
...
第一个修改是使用**分而治之**设计策略。前一版本将![](assets/03b1d302-6d22-41ac-8afd-1daf82c052c4.png)的计算分为![](assets/3dc4db35-140a-4a09-ba3f-a39e9ba98f7e.png)步；循环执行了*n*个乘法运算。如果我们能找到一种方法将问题分成两个相等的部分，那么问题将分解为![](assets/4381448c-6b9b-4d8c-83eb-142b157d1a6a.png)步骤。

例如，`Power1`可调用的`pow1(2,1024)`执行 1024 个单独的乘法运算。我们可以将其优化为 10 次乘法，这是一个显著的加速。

我们将使用*快速求幂*算法，而不是简单地乘以固定值。它使用三个基本的计算规则，如下所示：

*   如果为![](assets/55748872-6fda-4bdb-9ca2-033792dd0084.png)，则为![](assets/f45b0135-35ae-4b2f-9944-d9e696529866.png)，结果为 1。
*   如果*n*为奇数，![](assets/754eb5c1-31c1-44d5-bf72-e2a253c9b9aa.png)，结果为![](assets/4d64564c-fc50-421b-8b89-e4f41ab81c3a.png)。这涉及到对![](assets/8a76ed37-b3f0-4015-a11b-c8142fe97ccb.png)的递归计算。这是一次乘法。然而，![](assets/eb6ccd89-6c3b-4e55-a7ab-550b3238cbc8.png)是一个偶数，可以进行优化。
*   如果*n*为偶数，![](assets/8f645e52-5b14-4671-a180-4bbea8b34854.png)，则结果为![](assets/1f4e65d0-2eeb-4ec1-ac47-92e042ad3a8d.png)。这涉及到对![](assets/d9ae7819-da3d-47ff-b96f-f965192c0f57.png)的递归计算。这将乘法的数目减半。
...
*   如果*n*为零，我们将返回 1。
*   如果*n*为奇数，我们将进行递归调用并返回![](assets/fe094ca4-5a18-4c3e-a50b-752c69a18c52.png)。
*   如果*n*为偶数，我们将进行递归调用并返回![](assets/febc81e7-2f24-43a0-9676-968496c983bb.png)。
...
否则，缓存中不存在参数值。在这种缺失值的情况下，必须计算并保存![](assets/a727c0ee-d1fe-4927-b623-03085d56a616.png)的值。计算快速指数的三个规则用于获取值并将值放入缓存。这使我们确信，未来的计算将能够利用缓存的值。
