...
RST 内联标记包括多种常见的文本 ASCII 处理方法。例如，我们有`*emphasis*`和`**strong emphasis**`，它们通常分别产生斜体和粗体。我们可能希望强调文本块中的代码段；我们使用```literal```强制使用单间距字体。
...
如果我们在文档字符串中使用```SomeClass```，我们将以单间距字体获得类名。如果我们使用`:py:class:`SomeClass``，我们会得到一个到类定义的适当链接，这通常会更有帮助。
...
通过使用`:py:class:`Card``而不是```Card```，我们能够在这个注释块和`Card`类的定义之间创建显式链接。虽然包含角色前缀会有一点键入开销，但由此产生的交叉引用网络会产生非常有用的文档。
