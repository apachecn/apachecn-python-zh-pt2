...
定义一个新的数字类的唯一原因是使算术运算符过载。每个`FixedPoint`对象有两部分：`value`和`scale`，我们可以说某个值![](assets/562770d9-a620-4d9f-b849-00118e0b232e.png)是![](assets/b7d94d0a-3738-474a-93a8-33c3a58323ee.png)值除以比例因子![](assets/83807ad9-ef1e-4833-ad85-669a81bd97d3.png)：![](assets/143ad21c-2291-44b1-a3bd-21cf95cde76f.png)的一部分。

请注意，在下面的示例中，我们已经使用正确但低效的浮点表达式计算出了代数，我们将讨论效率稍高的纯整数运算。

加法（和减法）的一般形式是：![](assets/9f206058-438d-4c56-929d-fe46d075f049.png)，它可以创建一个具有许多无用精度的结果。

想象一下添加 9.95 和 12.95。我们（原则上）有 229000/10000。这可以适当地减少到 2290/100。问题是它也降低到 229/10，不再是美分。我们希望避免以一般方式减少分数，而是尽可能坚持使用美分或米尔斯。

我们可以为![](assets/56973b74-bf75-4f76-9113-1296a1bfd0d7.png)确定两种情况：

*   **比例因子匹配**：本例中为![](assets/cdc82905-eb17-4c19-9542-fb31e791ada0.png)，总和为![](assets/08278979-236c-4fe8-99e2-1101d7ebf72e.png)。当添加一个`FixedPoint`值和一个普通的旧整数值时，这也会起作用。我们可以强制整数具有所需的比例因子。

*   **比例因子不匹配**：当![](assets/7be7650e-c0a3-49ef-bb6c-9bf176a8e7b0.png)时，可以帮助生成两个输入值中比例因子最大的结果量表![](assets/c7aaa0fd-0fce-4f7a-bf1b-cef8e4cb7002.png)：![](assets/b9538206-ee94-4cf0-a010-089a8ceb3cb3.png)。由此，我们可以计算出两个中间尺度因子：![](assets/bc26f496-8103-446e-a50d-4f77322336e8.png)和![](assets/a60a8dc7-d66a-49a9-92b4-b17aff73db00.png)。其中一个比例因子为 1，另一个小于 1。我们现在可以在分母中添加一个公共值。在代数上，它是![](assets/3a8c67c0-79b6-42b0-9b20-c6d891b2da74.png)。这可以进一步优化为两种情况，因为其中一个因素是 1，另一个是 10 的幂。

我们不能真正优化乘法。是![](assets/ed9a41d0-471e-4718-b377-cdd93c5028d5.png)。当我们乘以`FixedPoint`值时，精度确实必须提高。

除法是用一个倒数进行乘法：![](assets/ce96348d-5aad-43c9-8dd9-4c4ca1e4c7bf.png)。如果 A 和 B 具有相同的比例，这些值将取消，这样我们就有了一个方便的优化。然而，这将比例从美分改为整份，这可能不合适。
...
首先，我们可以修改这个类的`__new__()`方法，以确保刻度非零，从而消除对`sys.hash_info.inf`的任何需要。其次，我们可以明确地将比例因子的范围限制为小于`sys.hash_info.modulus`（对于 64 位计算机，通常为![](assets/44dbe26c-3607-416c-b594-73f0eccda4d7.png)。我们可以消除消除`P`共同因素的需要。这将把散列归结为`hash_ = (abs(m) % P) * pow(n, P - 2, P) % P`、符号处理和特例-1 映射到-2。
