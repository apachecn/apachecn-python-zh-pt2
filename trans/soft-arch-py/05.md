...
uWSGI 是一个开源项目和应用，旨在为托管服务构建一个完整的堆栈。uWSGI 项目的 WSGI 源于一个事实，即用于 Python 的WSGI 接口插件是该项目中开发的第一个插件。
...
uWSGI是 Python web 应用生产部署的理想选择，在这种部署中需要在定制与高性能和特性之间取得良好的平衡。它是 WSGI web 应用部署组件的瑞士军刀。
