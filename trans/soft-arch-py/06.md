...
让我们假设一个场景，攻击者试图利用这段代码找出应用程序正在运行的目录的内容。（目前，您可以假设攻击者可以通过 web 应用程序运行此代码，但无法直接访问机器本身）。
...
这通过传入服务器无法处理的 CPU 密集型计算打开了简单 DoS 攻击的途径。例如，在以下攻击中，我们传入了一个非常大的数字计算，它占用了系统的 CPU，降低了系统速度，并使应用程序无响应：
