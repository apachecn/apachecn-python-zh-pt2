...
Po.T0:Server 端模板注入 Oracle T1（State T2AsSSTI OUTT3）是使用通用 Web Ty4T4 框架的服务器端模板作为攻击向量的攻击。该攻击利用了用户输入嵌入模板的方式中的弱点。SSTI 攻击可以用来找出 web 应用的内部结构，执行 shell命令，甚至完全破坏服务器。
