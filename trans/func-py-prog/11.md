...
修饰函数的好处之一是我们可以创建复合函数。这些是包含来自多个来源的功能的单个功能。复合函数![](images/f30bd4ad-cfc8-4f3d-9d34-52739b7d4958.png)比![](images/68cf946b-a3f5-43a7-9517-a3a911959f3c.png)更能表达复杂算法。对于表达复杂的处理，有许多语法选择通常是有帮助的。
...
![](images/0ff9f75e-388d-4fc7-b382-d7b737093f80.png)

我们可以定义一个新函数![](images/fa927ce9-7efa-440e-a7a3-e4e75fe0270b.png)，它结合了两个其他函数![](images/96f46537-f4f3-4dc1-8c6e-b508333f6298.png)和![](images/e33d92cc-d8ed-4350-851d-afa688e1b580.png)。
...
由此产生的函数本质上等同于![](images/2823d0a2-a722-4e2c-bc31-db0c3c347f81.png)。`f_deco()`装饰器必须通过将`f()`的内部定义与提供的`g()`合并来定义并返回复合函数。

实现细节显示 Python 实际上提供了一种稍微复杂一点的组合。包装器的结构有助于将 Python decorator 组合考虑如下：

![](images/8fa2a999-288d-4209-8a21-b06c622dbd39.png)

应用于某个应用程序函数![](images/e714ba82-e405-4638-a885-ac43f5dc0f53.png)的装饰器将包括一个包装函数![](images/e1f35afa-6fb6-4570-b340-9f225f6e23b7.png)，它由两部分组成。包装的一部分![](images/6b2ee252-bac1-4c9e-b5a7-59717c22f3ba.png)适用于包装函数![](images/3a45e2e1-1717-4764-a822-2d462ab4e803.png)的参数，另一部分![](images/9f2fe420-9c64-44fb-a2d3-79966fd9b2cc.png)适用于包装函数的结果。
...
这显示了在原始函数之前和之后插入附加处理的位置。这强调了函数组合的抽象概念和 Python 实现之间的一个重要区别：装饰器可以创建![](images/54abd952-fec7-45c4-96ab-c09e5e6430ac.png)或![](images/43a3838d-bb37-426f-8467-25d06ac3ac9a.png)或更复杂的![](images/1af76d48-a5bf-4eaf-8a63-51731d79be86.png)。装饰的语法没有完全描述将创建哪种类型的构图。
...
大量函数式编程遵循![](images/0ff9f75e-388d-4fc7-b382-d7b737093f80.png)设计模式。从两个较小的函数定义组合并不总是有帮助的。在某些情况下，将这两个功能分开可以提供更多信息。然而，在其他情况下，我们可能需要创建一个复合函数来总结处理过程。
...
一个常见的需求是使用附加参数自定义装饰器。我们可以做一些更复杂的事情，而不是简单地创建一个复合![](images/76a80762-1dd3-45c4-b71f-da14d491c183.png)。通过参数化装饰器，我们可以创建![](images/bad38d5c-dbc3-4a05-8eda-2fb4c8343539.png)。我们已经应用了一个参数*c*，作为创建包装器![](images/742f3a5e-a789-4934-85cd-1525559ccee6.png)的一部分。这个参数化的复合函数![](images/db7c49a6-ec2d-4b9c-a175-6cc9f6fed21b.png)可以与实际数据*x*一起使用。
...
Python 中没有任何东西可以阻止我们堆积修改其他 decorator 结果的 decorator。这有点像![](images/5c89994e-c53c-4487-bbd0-c19e310e5bf3.png)的意思。但是，结果名称将仅为![](images/f0fc382d-4f7b-41fd-b3c8-1c070dca61aa.png)。由于这种潜在的混淆，我们在创建涉及深度嵌套装饰器的函数时需要谨慎。如果我们的目的仅仅是处理一些交叉关注点，那么每个装饰器都应该设计为处理一个单独的关注点，而不会造成太多混乱。
...
尝试转换和重试的第三部分是实际的包装器，它也是复合函数的一部分。如前所述，包装器包含一个参数阶段和一个返回值，我们分别称之为![](images/5a118d10-48c2-4e3f-9d3a-bdfd11047bdb.png)和![](images/3a127cd0-8e04-4af7-a156-7be697064078.png)。
