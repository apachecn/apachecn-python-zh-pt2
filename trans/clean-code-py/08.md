...
假设我们按照SOLID原则设计一个软件，其中一部分实际上有一组符合开放/封闭原则的组件，这意味着我们可以轻松地扩展它们，而不会影响太多现有代码。进一步假设代码的编写方式有利于重构，因此我们可以根据需要对其进行更改。当我们进行这些更改时，我们没有引入任何 bug，这是怎么说的？我们如何知道现有功能得到了保留（并且没有退化）？您是否有足够的信心将其发布给您的用户？他们会相信新版本的效果和预期的一样吗？
